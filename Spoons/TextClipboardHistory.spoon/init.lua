...
function obj:_processSelectedItem(value)
   ...
   if value and type(value) == "table" then
      ...
      -- MODIFIED: Check for `full_text` instead of `text` to perform the paste action.
      elseif value.full_text then
         -- MODIFIED: Set the pasteboard to the full text content.
         pasteboard.setContents(value.full_text)
         last_change = pasteboard.changeCount()
         -- Re-insert at the top of the history, as if it had just been copied again.
         -- dedupe_and_resize will drop the older occurrence since this one is now first.
         self:pasteboardToClipboard({full=value.full_text, preview=value.text})
...
function obj:dedupe_and_resize(list)
   ...
   for i,v in ipairs(list) do
      if #res < self.hist_size then
         -- MODIFIED: We now hash the FULL text for deduplication, not the preview.
         local hash=hashfn(v.full)
...
function obj:pasteboardToClipboard(item)
   -- MODIFIED: item is now expected to be a table: {full="...", preview="..."}
...
function obj:_populateChooser()
   ...
   for k,v in pairs(clipboard_history) do
      -- MODIFIED: Check for the new table structure.
      if (type(v) == "table" and v.full and v.preview) then
         -- MODIFIED: The chooser row now displays the `preview` but carries the `full_text`
         -- which will be used when the item is selected.
         table.insert(menuData, {text=v.preview, full_text=v.full})
      end
   end
   if #menuData == 0 then
...
function obj:checkAndStorePasteboard()
   ...
   if (now > last_change) then
      if (not self.honor_ignoredidentifiers) or self:shouldBeStored() then
         ...
         elseif current_clipboard ~= nil and current_clipboard ~= "" then
            -- Create a preview snippet
            local preview = current_clipboard:sub(1, 150)
            if #current_clipboard > 150 then
               preview = preview .. " ..."
            end
            self.logger.df("Adding to clipboard history: %s", preview)
            -- MODIFIED: Store a table with both the full content and the preview.
            self:pasteboardToClipboard({full=current_clipboard, preview=preview})
         else
            self.logger.df("Ignoring nil or empty clipboard content")
         end
      else
